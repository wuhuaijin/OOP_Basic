...
int tototo(string number){
    int lens = number.length();
    int sum = 0;
    bool flag = 1;
    if (!((number.at(0) >= '0' && number.at(0) <= '9') || number.at(0) == '-')) error("INVALID NUMBER");
    else {
        if (number.at(0) == '-') flag = 0;
        else sum = number.at(0) - '0';
    }

    for (int i = 1; i < lens; ++i)
    {
        if (number.at(i) < '0' || number.at(i) > '9') error("INVALID NUMBER");
        else sum = number.at(i) - '0' + sum * 10;
    }
    if (flag) return sum;
    else return (-sum);
}
